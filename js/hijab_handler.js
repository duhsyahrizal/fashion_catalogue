...
const productsHijab = document.querySelector(".product-hijab-dom");
...
class UIAll {
  displayProducts(data){
    ...
    products.forEach(product => {
      ...
      if(product.category.name == 'hijab'){
        result += `
        <div class="col mb-4">
          <div class="card">
    ...
    productsHijab.innerHTML = result;
  }
  getFavButtons(){
    const buttons = [...document.querySelectorAll('.fav-btn')];
    buttonsDOM = buttons;
  ...
  addFavItem(item){
    ...
    div.innerHTML = `
      ...
      <div class="col-lg-7 col-md-6 col-sm-8 col-7 mb-3">
        ...
        <div class="item-price"><a href="detail?product=${item.title}&category=${item.category.name}" data-id="${item.id}" id="product-id">Checkout</a></div>
      </div>
